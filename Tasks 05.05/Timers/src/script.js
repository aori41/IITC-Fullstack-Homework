...
const timerButton = document.querySelector("#sec-timer-button");
const secTimer = document.querySelector("#sec-timer");

timerButton.addEventListener("click", startTimer);

function startTimer() {
    let time = +secTimer.value;

    if (time > 0) {
        timerButton.removeEventListener("click", startTimer);

        secondDiv.innerText = "Timer has been started!";

        const timer = setInterval(() => {
            secondDiv.innerText = time;
            time--;

            if (secondDiv.innerText === "0") {
                clearInterval(timer);
                timerButton.addEventListener("click", startTimer);
            }
        }, 1000);
