...
function playerAction() {
	...
	while (!positionId ||
		...
		positionId = prompt(`${(turn === player1.symbol) ? player1.name : player2.name}'s turn!\n`
			+ `Playing as: ${(turn === player1.symbol) ? player1.symbol : player2.symbol}\n`
			+ "Write coords of the location you want\n"
			+ "place your symbol as: height-width");

		position = positionId.split("-");
	}
	board[position[0] - 1][position[1] - 1] = (turn === player1.symbol) ? player1.symbol : player2.symbol;
...
function checkEndGame() {
	...
	if (win) {
		alert(`${(turn === player1.symbol) ? player2.name : player1.name} is the Winner!`);
		return true;
	}
